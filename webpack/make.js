...
function make(conf) {
  ...
  const extract = new ExtractTextPlugin({filename: 'styles.css', allChunks: true});
  ...
  // Alteration of loaders Conditional for server/client or development/production
  // Modify with care.
  Object
    .keys(loaders)
    .map(key => loaders[key])
    .forEach(loader => {
      ...
      // Replace style loaders with extract plugin on client in production Detach
      // classnames and style loader outside extract. But add style loader as
      // fallback.
      ...
      // Don't emit files from file-loader on the server. This speeds things up.
  ...
  if (isClient) {
    // Set entry point
    config.entry.client = ['regenerator-runtime/runtime', './src/client/index.jsx'];
    ...
    config
      .plugins
      .push(new webpack.optimize.CommonsChunkPlugin({name: 'vendor', minChunks: Infinity, filename: 'vendor.js'}),
