...
const debug = require('../src/server/utils/debug');
const clientConfig = require('./client');
const serverConfig = require('./server');

const domain = [
  color.magentaBright('webpack'),
  '::',
  color.redBright('server')
].join('');
const domainClient = [
  color.magentaBright('webpack'),
  '::',
      ...
      if (sample) {
        ...
        if (sample === '.js') {
          loaders[0]
            .query
            .plugins
            .splice(0, 0, 'react-hot-loader/babel');
        }
        clientConfig
          .plugins
          .push(new HappyPack({id, loaders, threadPool, verbose: false}));
        delete res.loader;
...
clientConfig
  .entry
  .client
  .splice(0, 0, 'react-hot-loader/patch', `webpack-hot-middleware/client?reload=true&path=http://localhost:${proxyPort}/__webpack_hmr`);

// Add ProgressBar, HMR and NoErrors plugin
clientConfig
  .plugins
  .push(new ProgressBarPlugin({
    width: 12,
    format: `[:bar] ${domainClient} ${color
      .green
      .bold(':percent')} :msg (:elapsed seconds)`,
    clear: true,
    summary: false
...
serverConfig
  .plugins
  .push(new webpack.NoErrorsPlugin());

// Performance hints
clientConfig.performance = {
  hints: false
};
  ...
  build
    ...
    .on('connection', (connection) => {
      ...
      connection.on('close', () => {
        build
          .connections
          .splice(build.connections.indexOf(connection));
      });
    });
});
